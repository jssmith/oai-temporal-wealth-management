...
async def main():
    ...
    try:
        conversation_id = input("Enter conversation ID (or press Enter for new conversation): ").strip()
    ...
    try:
        ...
        if existing_items:
            print(f"Loaded {len(existing_items)} previous messages from conversation history.")
        ...
        # Clean up session
        session.close()
        print(f"Conversation {conversation_id} saved. You can resume it later by entering the same ID.")
