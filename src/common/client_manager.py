...
class ClientManager:
    ...
    def get_client(self, client_id: str) -> dict:
        try:
            ...
            return {
                "error": f"Exception occurred while retrieving Client {client_id} error: {e}"
            }
...
def main():
    ...
    parser.add_argument(
        "--client-id", type=str, required=True, help="The ID of the client to manage"
    )
    ...
    if args.add:
        ...
        if not all(all_arguments):
            parser.error(
                "--add requires --first-name, --last-name, --address, --phone, --email and --marital_status."
            )
        ...
        if not any(all_arguments):
            parser.error(
                "--update requires at least one of --first-name, --last-name, --address, --phone, --email or --marital_status."
            )
        args_dict = vars(args)
        ...
        update_dict = {
            ...
            if v is not None and not isinstance(v, bool) and k != "client_id"
        }
        # print(f"Update dictionary looks like this: {update_dict}")
        result = client_manager.update_client(
            client_id=args.client_id, new_info=update_dict
        )
