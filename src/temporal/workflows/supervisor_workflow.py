...
from common.user_message import ProcessUserMessageInput, ProcessUserMessageResponse, ChatInteraction
...
from src.temporal.activities.open_account import OpenAccount, open_new_investment_account
...
class WealthManagementWorkflow:
    ...
    def __init__(self):
        ...
        self.session = TemporalDB2Session(
            session_id=f"wf_{self.wf_id}"
        )
    ...
    def _init_agent_graph(self) -> dict[str, Agent[WealthManagementContext]]:
        ...
        workflow.logger.info(f"Initialized agent graph with agents: {list(agents.keys())}")
        return agents
    ...
    def _get_agent_by_name(self, agent_name: str) -> Agent[WealthManagementContext]:
        ...
        workflow.logger.warning(f"Agent '{agent_name}' not found, defaulting to supervisor")
    ...
    def _initialize_claim_check_codec(self):
        ...
        try:
            ...
            workflow.logger.info(f"Initialized claim check codec for workflow {self.wf_id}")
    ...
    async def _cleanup(self):
        ...
        if self.claim_check_codec:
            try:
                ...
                workflow.logger.info(f"Cleaned up claim check payloads for workflow {self.wf_id}")
            except Exception as e:
    ...
    async def run(
        ...
        workflow.logger.info(f"Agent graph initialized")
        ...
        while True:
            ...
            try:
                ...
                chat_interaction = await self._process_user_message_request(req.message.user_input)
                
                # Update workflow details with recent conversation
                #await self._update_workflow_details()
                
                # Complete the request with success
                req.complete(ProcessUserMessageResponse(
                    chat_interaction=chat_interaction,
                    success=True
                ))
                workflow.logger.info(f"Request completed successfully")
                ...
                workflow.logger.info(f"Request completed with guardrail block")
                ...
                req.complete(ProcessUserMessageResponse(
                    chat_interaction=error_chat,
                    success=False,
                    error_message=str(e)
                ))
                workflow.logger.info(f"Request completed with error")
    ...
    @workflow.update
    async def process_user_message(self, message_input: ProcessUserMessageInput) -> ProcessUserMessageResponse:
        """
        Queue a user message for processing and wait for the response.
        The actual processing happens in the workflow run loop.
        """
        workflow.logger.info(f"Update handler: Queueing user message: {message_input.user_input}")
        ...
        workflow.logger.info(f"Update handler: Received response for message")
        return response
