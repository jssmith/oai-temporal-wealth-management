...
class DB2SessionActivities:
    ...
    async def get_items(
        ...
        activity.logger.info(
            f"get_items: Retrieving items for session {session_id} (limit={limit})"
        )
    ...
    async def add_items(
        ...
        activity.logger.info(
            f"add_items: Adding {len(items)} items to session {session_id}"
        )
        ...
        try:
            ...
            activity.logger.info("add_items: Successfully added items")
        finally:
            session.close()
    ...
    async def clear_session(
        ...
        try:
            ...
            activity.logger.info("clear_session: Session cleared successfully")
        finally:
            session.close()
