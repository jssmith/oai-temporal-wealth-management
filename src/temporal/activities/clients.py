...
class ClientActivities:
    ...
    async def add_client(new_client: WealthManagementClient) -> str:
        activity.logger.info(
            f"add_account. input: {new_client.first_name} "
    ...
    @staticmethod
    @activity.defn
    async def get_client(client_id: str) -> Optional[WealthManagementClient]:
        ...
        if "error" in client_dict:
            print(
                f"Error after trying to get client {client_id}. {client_dict['error']}"
            )
            return None
