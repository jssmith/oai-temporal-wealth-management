...
from src.temporal.activities.clients import WealthManagementClient
...
class OpenAccount:
    ...
    async def get_current_client_info(workflow_id: str) -> WealthManagementClient:
        ...
        client = await handle.execute_update(
    ...
    async def update_client_details(
        workflow_id: str, client_details: WealthManagementClient
    ) -> str:
        # get the handle from the workflow id
        handle = await OpenAccount.get_workflow_handle(workflow_id)
