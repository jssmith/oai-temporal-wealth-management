...
class ClaimCheckPlugin(Plugin):
    def __init__(self):
        ...
        # TTL configuration for claim check payloads (default 60 days)
        self.ttl_hours = int(os.getenv("CLAIM_CHECK_TTL_HOURS", "1440"))

        # Compression configuration
        self.enableCompression = str_to_bool(
            os.getenv("CLAIM_CHECK_COMPRESSION", "True")
        )
        self.compressionThreshold = int(
            os.getenv("CLAIM_CHECK_COMPRESSION_THRESHOLD", "250")
        )
    ...
    def get_data_converter(self, config: ClientConfig) -> DataConverter:
        ...
        if self.useClaimCheck:
            ...
            print(
                f"compression enabled: {self.enableCompression}, threshold: {self.compressionThreshold} bytes"
            )
            claim_check_codec = ClaimCheckCodec(
                config=self.db2_config,
                ttl_hours=self.ttl_hours,
                enable_compression=self.enableCompression,
                compression_threshold=self.compressionThreshold,
            )
