...
class DB2Session(SessionABC):
    ...
    def _ensure_session_exists_sync(self):
        ...
        try:
            ...
            check_sql = f"SELECT session_id FROM {self.sessions_table} WHERE session_id = ?"
    ...
    def _clear_session_sync(self) -> None:
        ...
        try:
            ...
            delete_messages_sql = f"DELETE FROM {self.messages_table} WHERE session_id = ?"
            stmt = ibm_db.prepare(conn, delete_messages_sql)
            ibm_db.bind_param(stmt, 1, self.session_id)
            ibm_db.execute(stmt)
            
            # Delete session record
            delete_session_sql = f"DELETE FROM {self.sessions_table} WHERE session_id = ?"
